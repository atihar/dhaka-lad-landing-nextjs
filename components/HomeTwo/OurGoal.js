export default function OurGoal() {
	return (
		<section className="promo-content-block fix section-padding section-bg">
			<div className="container">
				<div className="row align-items-center">
					<div className="col-xl-6 col-12 mt-5 mt-xl-0 order-2 order-xl-1">
						<div className="block-contents">
							<div className="section-title mb-4">
								<h2 className="wow fadeInUp">Our goal is to avail subscriptions to everyone</h2>
								<p className="wow fadeInUp pt-15" data-wow-delay=".3s">
									At Dhaka Lad, we are driven by the belief that access to premium subscriptions should be a 
									universal privilege, not a luxury reserved for a few. Our core mission is to break down 
									the barriers that stand between individuals and the high-quality digital tools they need 
									to thrive in today&apos;s fast-paced world.
								</p>
							</div>
						</div>
						<div className="funfacts d-flex">
							<div className="single-funfact wow fadeInUp" data-wow-delay=".5s">
								<h3>
									<span>290</span>+
								</h3>
								<p>Happy Subscribers</p>
							</div>
							<div className="single-funfact wow fadeInUp" data-wow-delay=".8s">
								<h3>
									<span>97</span>%
								</h3>
								<p>Subscribers satisfaction</p>
							</div>
						</div>
					</div>
					<div className="col-xl-5 offset-xl-1 order-1 order-xl-2">
						<img src="img/about-img1.jpg" alt="" />
					</div>
				</div>
			</div>
		</section>
	);
}
